...
def notify_support_human(user_id: str, text: str, suggestions):
    ...
    for i, s in enumerate(suggestions, 1):
        print(f"  Sugestão {i} (conf {s.confidence:.2f}):\n{s.text}\nAções: {s.actions}\n")
...
async def main():
    ...
    out = await router.dispatch("Pilot", Message("u123", "Quero gerar imagem de um corte de cabelo moderno"), session)
    print("BOT:", out.text)

    print("\n--- CENÁRIO 3: intenção de agendar (vai p/ especialista) ---")
    out = await router.dispatch("Pilot", Message("u123", "Quero agendar amanhã"), session)
    print("BOT:", out.text)

    print("\n--- CENÁRIO 4: pedir humano (escala p/ suporte) ---")
    out = await router.dispatch("Pilot", Message("u123", "Quero falar com um atendente humano"), session)
    print("BOT:", out.text)
