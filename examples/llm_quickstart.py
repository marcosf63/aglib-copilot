...
class MockHumanBridge:
    ...
    async def handle(self, msg, session):
        ...
        return AgentOutput(
            Action.REPLY,
            text="[Mock] Conectando com consultor humano..."
        )
...
async def setup_demo_environment():
    ...
    session.business_context.order_history = [
        {"id": "order1", "date": "2025-08-01", "total": 45.00, "service": "corte_masculino"},
        {"id": "order2", "date": "2025-07-15", "total": 70.00, "service": "corte_masculino + barba"}
    ]
...
async def test_llm_integration():
    ...
    print(f"   Pilot Agent configurado:")
    ...
    for i, scenario in enumerate(test_scenarios, 1):
        ...
        try:
            ...
            if metrics.total_requests > 0:
                print(f"   📊 Métricas: {metrics.successful_requests}/{metrics.total_requests} sucessos")
...
async def test_tools_directly():
    ...
    if result.status.value == "success" and result.result:
        ...
        print(f"   Cliente: {profile.get('name', 'N/A')} ({profile.get('tier', 'regular')})")
