...
class MockCRMDatabase:
        ...
        demo_customers = [
            {
                ...
                "preferences": {
                    ...
                    "favorite_service": "corte_masculino",
                },
    ...
    def create_support_ticket(
        self, user_id: str, subject: str, description: str, priority: str = "normal"
    ) -> Dict:
    ...
    def update_ticket_status(
        self, ticket_id: str, status: str, notes: str = ""
    ) -> Dict:
    ...
    def log_interaction(
        self, user_id: str, interaction_type: str, details: Dict
    ) -> Dict:
    ...
    def get_customer_insights(self, user_id: str) -> Dict:
        ...
        if last_order_date:
            days_since_last_order = (
                datetime.now() - datetime.fromisoformat(last_order_date)
            ).days
        ...
        return {
            ...
            "preferred_services": customer.get("preferences", {}).get(
                "favorite_service"
            ),
    ...
    def _identify_risk_factors(
        self, customer: Dict, orders: List[Dict], tickets: List[Dict]
    ) -> List[str]:
        ...
        if customer.get("last_interaction"):
            days_since_interaction = (
                datetime.now() - datetime.fromisoformat(customer["last_interaction"])
            ).days
        ...
        recent_tickets = [
            t
            for t in tickets
            if (datetime.now() - datetime.fromisoformat(t["created_at"])).days <= 30
        ]
...
class GetCustomerProfileTool(Tool):
    def __init__(self):
        super().__init__(
            name="get_customer_profile", description="Buscar perfil completo do cliente"
        )
...
class UpdateCustomerInfoTool(Tool):
    def __init__(self):
        super().__init__(
            name="update_customer_info", description="Atualizar informações do cliente"
        )
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        try:
            updated_profile = mock_crm.update_customer_info(
                parameters["user_id"], parameters["data"]
            )
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        try:
            orders = mock_crm.get_order_history(
                parameters["user_id"], parameters.get("limit", 10)
            )
...
class CreateSupportTicketTool(Tool):
    def __init__(self):
        super().__init__(
            name="create_support_ticket", description="Criar ticket de suporte"
        )
...
class GetUserTicketsTool(Tool):
    def __init__(self):
        super().__init__(
            name="get_user_tickets", description="Buscar tickets de suporte do usuário"
        )
    ...
    async def execute(self, parameters: Dict[str, Any]) -> Dict[str, Any]:
        try:
            tickets = mock_crm.get_user_tickets(
                parameters["user_id"], parameters.get("status")
            )
...
class LogInteractionTool(Tool):
    def __init__(self):
        super().__init__(
            name="log_interaction", description="Registrar interação com o cliente"
        )
