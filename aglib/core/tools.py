...
from .types import (
    ToolDefinition, ToolParameter, ToolCall, ToolResult, ToolStatus, 
    ToolError, ValidationRule
)
    ...
    async def execute_tool(
        ...
        try:
            ...
            try:
                ...
                self._log_execution(call_id, tool.name, parameters, "success", execution_time)
                ...
                execution_time = time.time() - start_time
                self._log_execution(call_id, tool.name, parameters, "timeout", execution_time)
            ...
            self._log_execution(call_id, tool.name, parameters, "error", execution_time, error_msg)
...
class ToolRegistry:
    ...
    def get_tools_for_agent(self, agent_type: str, allowed_categories: List[str]) -> List[Tool]:
    ...
    def to_openai_format(self, tool_names: List[str] = None) -> List[Dict[str, Any]]:
        ...
        if tool_names:
            tools_to_convert = [self._tools[name] for name in tool_names if name in self._tools]
        else:
