...
class ConversationHistory:
    ...
    def add_message(self, role: str, content: str, agent: Optional[str] = None, **metadata):
...
class BusinessContext:
    ...
    def get_customer_summary(self) -> str:
        ...
        if self.support_tickets:
            open_tickets = len([t for t in self.support_tickets if t.get("status") == "open"])
            if open_tickets > 0:
                summary += f" - {open_tickets} tickets em aberto"
        
        if self.appointments:
            upcoming = len([a for a in self.appointments if a.get("status") == "scheduled"])
            if upcoming > 0:
    ...
    def get_relevant_context(self, topic: str) -> str:
        ...
        if "agend" in topic.lower() and self.appointments:
            ...
            context_parts.append(f"Agendamentos recentes: {len(recent_appointments)} encontrados")
        
        if "pagam" in topic.lower() and self.payments:
            recent_payments = self.payments[-3:]
            context_parts.append(f"Pagamentos recentes: {len(recent_payments)} encontrados")
        
        if "suporte" in topic.lower() and self.support_tickets:
            open_tickets = [t for t in self.support_tickets if t.get("status") == "open"]
...
class SessionState:
    ...
    def cleanup_expired(self):
        ...
        expired_keys = [
            key for key, entry in self.temp_data.items()
...
class Session:
    ...
    def _format_conversation_history(self, max_messages: int = 6) -> str:
        ...
        for msg in recent_messages:
            role_label = {"user": "Cliente", "assistant": "Assistente"}.get(msg.role, msg.role)
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "conversation_history": [msg.to_dict() for msg in self.conversation_history.messages],
            "business_context": {
                "customer_profile": self.business_context.customer_profile.__dict__ if self.business_context.customer_profile else None,
    ...
    def from_dict(cls, data: Dict[str, Any]) -> Session:
        ...
        if "business_context" in data:
            ...
            session.business_context = BusinessContext(
                customer_profile=CustomerProfile(**bc_data["customer_profile"]) if bc_data.get("customer_profile") else None,
