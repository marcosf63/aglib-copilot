...
class PilotAgent(Agent):
    def __init__(self, name: Optional[str] = None, consultants: Optional[ConsultantHub] = None):
    ...
    async def _handle_without_llm(self, msg: Message, session: Session) -> AgentOutput:
        ...
        if any(x in t for x in ["gerar imagem", "criar imagem", "imagem de", "gera uma imagem"]):
            if self.consultants.has("Consultant:Image"):
                # Pilot conversa com o consultor e formata a resposta final
                result = await self.consultants.get("Consultant:Image").handle(msg, session)
                img_url = result.payload.get("image_url")
                return AgentOutput(
                    Action.REPLY,
                    text=f"Pronto! Gerei uma imagem para você: {img_url}"
                )
            else:
                return AgentOutput(Action.REPLY, text="Ainda não tenho um consultor de imagens configurado.")

        # pedido explícito por humano => escalar
        if "humano" in t or "atendente" in t or "pessoa" in t:
            return AgentOutput(Action.ESCALATE, text="Vou te encaminhar para o suporte humano.")

        # caso geral: deixar policy decidir o especialista
        return AgentOutput(Action.HANDOFF, text="Entendido. Vou te encaminhar para o melhor agente.")
