...
from ..core.types import (
    Message, AgentOutput, Action, AgentConfig, AgentCapabilities,
    ToolCall, ToolResult, AgentPerformanceMetrics, LLMUsage
)
...
class Agent(ABC):
    ...
    def _initialize_llm(self):
        ...
        try:
            ...
            logger.info(f"LLM initialized for agent {self.name}: {self.config.llm_model}")
    ...
    def _setup_tools(self):
        ...
        logger.info(f"Agent {self.name} has {len(self.available_tools)} tools available")
    ...
    async def _handle_with_llm(self, msg: Message, session: Session) -> AgentOutput:
        ...
        try:
            ...
            return await self._process_llm_response(llm_response, tool_results, msg, session)
        ...
        for tool_call_data in tool_calls:
            ...
            try:
                ...
                tool_call = ToolCall(
                    ...
                    parameters=json.loads(tool_call_data["arguments"]) if isinstance(tool_call_data["arguments"], str) else tool_call_data["arguments"]
                )
    ...
    async def _process_llm_response(
        ...
        if tool_results:
            return await self._handle_tool_results(llm_response, tool_results, msg, session)
    ...
    async def _handle_tool_results(
        ...
        for result in tool_results:
            if result.status.value == "success":
                content = f"Tool {result.tool_call_id} executed successfully: {result.result}"
            else:
    ...
    def _determine_action(self, response_text: str, session: Session) -> Action:
        ...
        if any(keyword in response_lower for keyword in ["escalar", "supervisor", "gerente", "não consigo"]):
            return Action.ESCALATE
        
        # Check for handoff keywords
        if any(keyword in response_lower for keyword in ["especialista", "transferir", "encaminhar"]):
            return Action.HANDOFF
        
        # Check for human bridge keywords
        if any(keyword in response_lower for keyword in ["humano", "pessoa", "atendente"]):
